...
def test_store_configuration_defaults_to_sqlite(tmp_path, monkeypatch) -> None:
...
def test_database_url_selects_postgres(monkeypatch) -> None:
    ...
    store = configured_job_store()

    assert isinstance(store, PostgresJobStore)
    assert store.database_url == "postgresql://example.invalid/database"
